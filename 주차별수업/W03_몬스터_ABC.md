...
| **그림을 바꾼 뒤 몸이 바닥 아래로 떨어짐** (동작마다 다르기도 함) | **그림 파일의 `Pivot` · `Pixels Per Unit` 이 기본값으로 바뀐 것.** 그림 한 장을 눌러 `Pivot` 이 `Center` 거나 PPU가 `100` 이면 이 경우 → [08. 자주 나오는 문제 **6번**](../기본설명/08_자주_나오는_문제.md) |
...
네이버 카페 업로드 - 제목 `2D게임제작_03주차_123456홍길동`
