...
> 본 수업 문서: **[W03 몬스터 A·B·C](W03_몬스터_ABC.md)** · 같은 형식의 주인공 편: **[W02 주인공 Inspector 항목 설명](<W02_플레이어_캐릭터2(인스팩터).md>)**

몬스터도 주인공처럼 Inspector 항목이 많습니다. **구조는 주인공과 거의 같으니 [주인공 편](<W02_플레이어_캐릭터2(인스팩터).md>)을 먼저 읽고 오면 훨씬 빠릅니다.**
...
**배열 다루는 법은 [주인공 편](<W02_플레이어_캐릭터2(인스팩터).md>)의 "배열을 다루는 법"과 완전히 같습니다.** 항목 이름 오른쪽 끝의 숫자가 장수이고, `+` / `−` 버튼으로 한 칸씩 늘리거나 줄입니다. 그림은 `Element 0`부터 채웁니다.
...
**본격적인 사운드 작업은 11주차입니다.** 다만 **지금도 비어 있지 않습니다** - 기본 소리가 이미 연결되어 있습니다. **본인 소리로 바꾸는 것**을 11주차에 합니다.

> **소리 파일은 이 화면에서 끌어다 넣는 것이 아닙니다.** `06_Audio\SFX\MonsterA` (B·C도 각 폴더) 에 **정해진 이름으로** 파일을 두면(확장자는 자유), `Tools > Class Template > Wire MonsterA SFX Clips` 가 위 칸들을 채워줍니다. 파일명 목록은 그 폴더의 `readme.txt`에 있습니다.

> ★ **몬스터는 주인공보다 한 단계가 더 있습니다.** Wire는 `ActionTest` 씬에 있는 몬스터까지만 채웁니다. 그 뒤에 몬스터를 선택하고 **`Apply Monster A Settings To Prefab`** 을 눌러야 **다른 씬의 몬스터에도** 반영됩니다. [2주차 실습 7절](<W02_플레이어_캐릭터3(실습).md>)의 주인공 순서에 이 한 단계를 더한 것입니다.
