...
> 예를 들어 공격 프레임을 4장에서 3장으로 줄였는데 `Attack 1 Damage Frame`이 그대로 `4`라면, **있지도 않은 4번째 프레임을 가리키게 됩니다.** 이때는 **마지막 그림(3번째)으로 밀려서** 판정이 나갑니다. 에러 없이 그럭저럭 맞는 것처럼 보여서 **틀린 줄 모르고 넘어가기 쉽습니다** - 의도한 타이밍인지 눈으로 확인하세요.
>
> **몬스터C의 `Attack1 Projectile Frame` 만은 다릅니다.** 밀리지 않고 **투사체가 아예 안 나갑니다.**
